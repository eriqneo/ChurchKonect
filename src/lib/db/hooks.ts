...
// ==========================================
// 5. usePrayers(role) Hook
// ==========================================
export function usePrayers(roleId: string, currentUserLocalId = 'user-cell-leader-michael') {
  const requests = useLiveQuery(async () => {
    const all = await db.prayerRequests.toArray();
    
    // Non-pastors cannot view sensitive prayer requests unless it's their own
    if (roleId === 'lead_pastor' || roleId === 'administrator') {
      return all;
    }
    
    return all.filter(req => !req.isSensitive || req.memberId === currentUserLocalId);
  }, [roleId, currentUserLocalId]);

  const myAssignments = useLiveQuery(async () => {
    return await db.prayerAssignments
      .where('intercessorId')
      .equals(currentUserLocalId)
      .toArray();
  }, [currentUserLocalId]);

  const submitPrayer = useCallback(async (category: string, content: string, isSensitive: boolean, urgency: 'low' | 'medium' | 'high', memberName = 'Sister Clara Oswald') => {
    const newRequest = createLocalRecord<PrayerRequestRecord>({
      memberId: currentUserLocalId,
      memberName,
      category,
      content,
      isSensitive,
  ...
  const triagePrayer = useCallback(async (localId: string, urgency: 'low' | 'medium' | 'high', isSensitive: boolean) => {
    const existing = await db.prayerRequests.where('localId').equals(localId).first();
    if (!existing || !existing.id) return;

    await db.prayerRequests.update(existing.id, {
      urgency,
      isSensitive,
      syncStatus: 'pending',
      updatedAt: new Date().toISOString()
  ...
  const assignPrayer = useCallback(async (requestId: string, intercessorId: string, intercessorName: string) => {
    const req = await db.prayerRequests.where('localId').equals(requestId).first();
    ...
    const newAssignment = createLocalRecord<PrayerAssignmentRecord>({
      requestId,
      intercessorId,
      intercessorName,
      prayerCount: 0,
    ...
    await db.prayerAssignments.add(newAssignment);
    triggerAutoSync();
  }, []);

  const incrementPrayerCount = useCallback(async (assignmentId: string) => {
    const existing = await db.prayerAssignments.where('localId').equals(assignmentId).first();
    if (!existing || !existing.id) return;

    await db.prayerAssignments.update(existing.id, {
  ...
  const addRhemaNote = useCallback(async (requestId: string, note: string) => {
    const req = await db.prayerRequests.where('localId').equals(requestId).first();
    if (!req || !req.id) return;

    await db.prayerRequests.update(req.id, {
      rhemaNotes: note,
      status: 'answered', // Automark as answered when rhema word is spoken
      syncStatus: 'pending',
      updatedAt: new Date().toISOString()
    });
    triggerAutoSync();
  ...
  const sealPrayer = useCallback(async (requestId: string) => {
    const req = await db.prayerRequests.where('localId').equals(requestId).first();
    if (!req || !req.id) return;
